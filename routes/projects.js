...
router.post('/', [protect, body('title').notEmpty().withMessage('Title is required')], async (req, res) => {
  try {
    ...
    const { title, description, category, status, progress, githubUrl, backendUrl, frontendUrl, liveLinks, tags, startDate, endDate } = req.body;

    const project = await Project.create({
      user: req.user._id, title, description, category, status, progress,
      githubUrl, backendUrl, frontendUrl, liveLinks, tags, startDate, endDate
    });
...
router.put('/:id', [protect, body('title').optional().notEmpty().withMessage('Title cannot be empty')], async (req, res) => {
  try {
    ...
    project = await Project.findByIdAndUpdate(
      req.params.id,
      { title, description, category, status, progress, githubUrl, backendUrl, frontendUrl, liveLinks, tags, startDate, endDate },
